						...
						<div class="grid grid-cols-2 gap-5 md:grid-cols-2 xl:grid-cols-4">
								@foreach ($peminjamans as $peminjaman)
										<div
												...
												<a href="{{ route('delivery-order.show', $peminjaman->id) }}" class="p-2">
														<div class="flex w-full flex-col">
																<span
																		class="@if ($peminjaman->status == 'SELESAI') bg-green-200 text-green-600 border-green-600
																			@elseif ($peminjaman->status == 'DRIVER_DALAM_PERJALANAN')
																...
																<p class="my-2 font-bold line-clamp-2">{{ ucfirst($peminjaman->kode_do) }} </p>
																<div class="mb-2 flex flex-wrap">
																		@if (!$peminjaman->preOrder->isEmpty())
																				@foreach ($peminjaman->preOrder as $key => $row)
																						<p
																...
																@if ($peminjaman->logistic)
																		<div class="flex">
																				...
																				<p class="mb-2 text-sm font-normal text-gray-700 line-clamp-1">
																						{{ ucfirst($peminjaman->logistic->nama) }}</p>
																		</div>
																		<div class="flex">
																				...
																				<p class="mb-2 text-sm font-normal text-gray-700 line-clamp-2">
																						[{{ ucfirst($peminjaman->kendaraan->plat_nomor) }}]
																						{{ ucfirst($peminjaman->kendaraan->merk) }}</p>
																		</div>
																@else
																...
																<p class="mb-2 text-xs font-normal text-gray-500">
																		{{ \App\Helpers\Date::parseMilliseconds($peminjaman->created_at) }}</p>
												</a>
												@can('ADMIN')
														<div class="relative hidden h-full w-full items-center justify-center group-hover:flex">
																<div
																		...
																		<form action="{{ route('delivery-order.destroy', $peminjaman->id) }}" method="POST">
																				@csrf
																				<button type="submit"
																		...
																		<a href="{{ route('delivery-order.edit', $peminjaman->id) }}"
