...
@section('content')
		...
		<h1 class="my-6 w-full text-center text-lg font-bold uppercase">Edit Proyek</h1>
		<form method="POST" action="{{ route('proyek.update', $proyek->id) }}" enctype="multipart/form-data">
				...
				<div class="mb-6 grid w-[80vw] gap-6 md:grid-cols-2">
						<div>
								<label for="nama_proyek" class="mb-2 block text-sm font-medium text-gray-900 dark:text-gray-300">Nama
										Proyek</label>
						...
						<div class="@can('admin') block @elsecan('project-manager') hidden @endcan">
								<label for="proyek_manager_id" class="mb-2 block text-sm font-medium text-gray-900 dark:text-gray-300">Set
										Manager</label>
								<select id="searchSetManager" name="set_manager_id" class="searchSetManager block w-full"></select>
						...
						<div class="flex w-full flex-col">
								...
								<select name="provinsi" id="country"
										...
										@foreach ($provinces as $province)
												<option value="{{ $province }}" {{ $province == $proyek->provinsi ? 'selected' : '' }}>
														{{ $province }}</option>
										@endforeach
								</select>
						</div>
						...
						<div>
								...
								<input type="number" id="longitude" step="any" value="{{ old('longitude', $proyek->longitude) }}"
										name="longitude"
										class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500"
